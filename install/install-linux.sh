...
#==============================================================================
#                             pg_fastbcp Installation Wizard
#==============================================================================
# This script automatically detects your PostgreSQL installation and installs
# the pg_fastbcp extension.

# Define the extension file names. The .dll is replaced by .so on Linux.
SO_FILE="pg_fastbcp.so"
CONTROL_FILE="pg_fastbcp.control"
SQL_FILE="pg_fastbcp--1.0.sql"

echo ""
echo "=========================================================="
echo "          pg_fastbcp Linux Installation Wizard"
echo "=========================================================="
echo ""
...
check_service_status() {
    ...
    # Check if a postgres process is running
...
if [ "$FILES_EXIST" = true ] && [ "$POSTGRES_RUNNING" = true ]; then
    ...
    echo "1. Stop the PostgreSQL service (e.g., 'sudo systemctl stop postgresql' or similar)."
...
if [ $? -ne 0 ]; then
    echo "Error copying '$SO_FILE'."
    echo "This may be a permissions issue. Ensure you run this script with 'sudo'."
    exit 1
fi

sudo cp "$CONTROL_FILE" "$SHARE_PATH"
if [ $? -ne 0 ]; then
    echo "Error copying '$CONTROL_FILE'."
    echo "This may be a permissions issue. Ensure you run this script with 'sudo'."
    exit 1
fi
...
echo "2. Then, connect to psql and execute the following command:"
